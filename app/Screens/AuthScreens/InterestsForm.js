...
const InterestsForm = ({ route }) => {
  ...
  const handleSubmit2 = async () => {
    if (selectedInterests) {
      const interestsIds = selectedInterests.map(
        (interest) => interest.intrestId
      );
      ...
      try {
        ...
        if (error.response) {
          if (error.response.status === 400) {
            ...
            seterrorFormAPI("Select any one of the interest");
  ...
  const GetAllInterestsData = async () => {
    try {
      ...
      if (error.response) {
        if (error.response.status === 400) {
          console.log("Error With 400.", "Select any one of the interest");
          seterrorFormAPI({ passwordForm: `${error.response.data.message}` });
        } else if (error.response.status === 401) {
          seterrorFormAPI({ passwordForm: `${error.response.data.message}` });
        } else if (error.response.status === 403) {
        ...
        <View style={styles.container}>
          <View style={styles.ContentBox}>
            ...
            <View style={{ alignItems: "start", flex: 0.5, marginLeft: 10 }}>
              ...
              <View style={{justifyContent:"center",display:"flex",alignItems:"center"}}>{errorFormAPI ? <Text style={{color:"red"}}>{errorFormAPI}</Text> :""}</View>
