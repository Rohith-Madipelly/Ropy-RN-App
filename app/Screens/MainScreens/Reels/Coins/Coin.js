...
const Coin = ({ imageSource, delay, trigger, index }) => {
  ...
  useEffect(() => {
    if (trigger) {
      ...
      translateY.value = withDelay(
        delay,
        withTiming(height + 20, { duration: 2500 }) // Move to bottom
      );
    }
  }, [trigger]); // Trigger the effect only when 'trigger' changes
  ...
  switch (index) {
    ...
    case 2:
      ...
      break;
    case 3:
      size = 30; // Fourth coin smaller size
      break;
    case 4:
      size = 20; // Fifth coin smaller size
      break;
    default:
